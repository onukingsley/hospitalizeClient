...
const AllLabTest = () => {
    ...
    const {labTest,allLabTest} = labStore()
    ...
    let displayedDrugs = labTest;
    if (filter === 'My Sales') displayedDrugs = labTest;
    else if (filter === 'Daily Sales') displayedDrugs = allLabTest;
    ...
    return (
        <div className="space-y-6">
            <PageHeader title="My Lab Tests" subtitle="Lab Tests by the scientist" breadcrumb={[{ label: 'Dashboard', path: '/pharmacy' }, { label: 'Drug Stock' }]} actions={<Button>Request Stock</Button>} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <Card className="p-4 flex items-center gap-4 cursor-pointer hover:shadow-md transition-all" onClick={() => setFilter('all')}>
                    ...
                    <div><p className="text-2xl font-bold">{labTest.length}</p><p className="text-sm text-muted-foreground">My Lab Tests</p></div>
                </Card>
                <Card className="p-4 flex items-center gap-4 cursor-pointer hover:shadow-md transition-all" onClick={() => setFilter('all')}>
                    ...
                    <div><p className="text-2xl font-bold">{allLabTest.length}</p><p className="text-sm text-muted-foreground">Daily Lab Test</p></div>
                </Card>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm">
                    ...
                    <tr>
                        <th className="text-left p-3 font-medium">Lab Title</th>
                    ...
                    {filteredDrugs.map(sale => {
                        const status = getStockStatus[sale.lab_test_payment_status];
                        return (
                            <tr key={sale?.drugId} onClick={()=>navigate(`/pharmacy/saleDetails?sales_id=${sale.id}`)} className="border-b hover:bg-muted/50 transition-colors">
                                <td className="p-3 font-medium">{sale?.lab_test_name}</td>
                                <td className="p-3 text-muted-foreground">{sale?.patient?.user?.name}- {sale?.patient?.user?.regID}</td>
                                <td className="p-3">{sale?.lab_test_amount.toLocaleString()}</td>
                                <td className="p-3">{sale?.lab_test_description}</td>
