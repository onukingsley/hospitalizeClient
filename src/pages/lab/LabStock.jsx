...
import {drugStore, labStore} from "../../store/store.jsx";
...
const LabStock = () => {
    ...
    const {labStock,labLowStock,labOutOfStock,labPendingStock, labRestockRequest,myLabRestockRequest,pendingLabRestockRequest,updateLabRestockRequest} = labStore()
    ...
    let displayedDrugs = labStock;
    if (filter === 'low') displayedDrugs = labLowStock;
    else if (filter === 'outOfStock') displayedDrugs = labOutOfStock;
    else if (filter === 'pending') displayedDrugs = labPendingStock;
    else if (filter === 'restockRequest') displayedDrugs = labRestockRequest;
    else if (filter === 'myRequest') displayedDrugs = myLabRestockRequest;
    else if (filter === 'pendingRequest') displayedDrugs = pendingLabRestockRequest;
    ...
    const handleSubmitRequest = async () => {
        ...
        try {
            const payload = {
                labStock_id: selectedRequest.id,
            ...
            axiosClient.post('/restockLabRequest', payload)
                .then(({data})=>{
                    toast.success('Lab restock submitted successfully');
                    setShowAddDialog(false);
                    updateLabRestockRequest(data.data)
                    alert(data.message)
    ...
    return (
        <div className="space-y-6">
            <PageHeader title="Lab Stock" subtitle="Inventory management and stock levels" breadcrumb={[{ label: 'Dashboard', path: '/lab' }, { label: 'lab Stock' }]} actions={<Button>Request Stock</Button>} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <Card className="p-4 flex items-center gap-4 cursor-pointer hover:shadow-md transition-all" onClick={() => setFilter('all')}>
                    ...
                    <div><p className="text-2xl font-bold">{labStock?.length}</p><p className="text-sm text-muted-foreground">Total Lab Equipments</p></div>
                </Card>
                <Card className="p-4 flex items-center gap-4 cursor-pointer hover:shadow-md transition-all" onClick={() => setFilter('low')}>
                    ...
                    <div><p className="text-2xl font-bold text-red-600">{labLowStock?.length}</p><p className="text-sm text-muted-foreground">Low Stock</p></div>
                </Card>
                <Card className="p-4 flex items-center gap-4 cursor-pointer hover:shadow-md transition-all" onClick={() => setFilter('outOfStock')}>
                    ...
                    <div><p className="text-2xl font-bold text-yellow-600">{labOutOfStock?.length}</p><p className="text-sm text-muted-foreground">Out Of Stock</p></div>
                </Card>
                <Card className="p-4 flex items-center gap-4 cursor-pointer hover:shadow-md transition-all" onClick={() => setFilter('pending')}>
                    ...
                    <div><p className="text-2xl font-bold text-yellow-600">{labPendingStock?.length}</p><p className="text-sm text-muted-foreground">Pending</p></div>
                </Card>
                <Card className="p-4 flex items-center gap-4 cursor-pointer hover:shadow-md transition-all" onClick={() => setFilter('pending')}>
                    ...
                    <div><p className="text-2xl font-bold text-yellow-600">{labRestockRequest?.length}</p><p className="text-sm text-muted-foreground">Restock Request</p></div>
                </Card>
            </div>

            <Card className="p-4">
                <div className="flex gap-2 mb-4">
                    {['all', 'low', 'outOfStock','pending','restockRequest','pendingRequest','myRequest'].map(f => (
                        <Button key={f} variant={filter === f ? 'default' : 'outline'} size="sm" onClick={() => setFilter(f)}>
                            {f === 'all' ? 'All' : f === 'low' ? 'Low Stock' : f === 'outOfStock' ? 'outOfStock' : f === 'restockRequest' ? 'restockRequest' : f === 'myRequest' ? 'myRequest' : f=== 'pendingRequest' ? 'pendingRequest'  : 'New Pending'}
                        </Button>
                    ))}
                </div>
                        ...
                        {filteredDrugs?.length > 0 && filteredDrugs?.map(drug => {
                            ...
                            return (
                                <tr key={drug.drugId} className="border-b hover:bg-muted/50 transition-colors">
                                    ...
                                    }} className="p-3 font-medium">{drug.lab_stock.name}</td>
                                    <td className="p-3 text-muted-foreground">{drug.lab_stock.name}</td>
                                    <td className="p-3"><Badge variant="outline" className="text-xs">{drug.user.name}</Badge></td>
                                    <td className="p-3">{drug.quantity} units</td>
                                    <td className="p-3">{drug?.lab_stock.amount ? drug.lab_stock.amount.toLocaleString() : 0}</td>
                        ...
                        {filteredDrugs?.map(drug => {
                            ...
                            return (
                                <tr key={drug.drugId} className="border-b hover:bg-muted/50 transition-colors">
                                    ...
                                    <td className="p-3 text-muted-foreground">{drug.name}</td>
                                    <td className="p-3"><Badge variant="outline" className="text-xs">{drug.name}</Badge></td>
                                    <td className="p-3">{drug.quantity} units</td>
                                    <td className="p-3">{drug?.amount ? drug.amount.toLocaleString() : 0}</td>
                                    <td className="p-3">{formatDate(drug.status)}</td>
                                    <td className="p-3"><span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium border ${status.color}`}>{drug.status}</span></td>
                                    {(drug.status == 'outOfStock' || drug.status == 'lowStock') && (
                                        <td className="p-3">
                                            <Button
                                                variant="ghost"
                                                size="sm"
                            ...
                            <ScrollArea className="flex-1 pr-4">
                                <div className="space-y-6 py-4">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <p className="font-semibold text-lg">
                                                    {selectedRequest?.name ?? selectedRequest?.lab_stock.name ?? 'N/A'}
                                                </p>
                                            ...
                                            <div className="text-right">
                                                ...
                                                <p className="text-xs text-muted-foreground">
                                                    Current Stock: {selectedRequest.lab_stock?.quantity || 'N/A'}
                                                </p>
                                            </div>
                                        </div>
                                    </Card>
                                    ...
                                    <Card className="p-4">
                                        ...
                                        <p className="text-sm mt-1">{selectedRequest.description ?? selectedRequest.lab_stock.description}</p>
                                    </Card>
                        ...
                        <>
                            ...
                            <ScrollArea className="flex-1 pr-4">
                              ...
                              {selectedRequest.quantity}
                                                ...
                                                <p className="text-xs text-muted-foreground">
                                                    Current Stock: {selectedRequest.quantity || 'N/A'}
                                                </p>
                                            </div>
                                        </div>
                                    </Card>
                            ...
                            <DialogFooter className="border-t pt-4 flex-shrink-0 flex-wrap gap-2">
                                ...
                                <Button variant="primary" onClick={() => setShowRestockDialog(false)}>
                                    cancel
                                </Button>
                                <Button variant="outline" onClick={() => handleSubmitRequest()}>
                                    Request
                                </Button>
                            </DialogFooter>
                        </>
