...
const StaffManagement = () => {
  ...
  const navigate = useNavigate()
    ...
    const {accountants,totalAccountants,doctors,totalDoctors,clerks,totalClerks,nurses,totalNurses,pharmasists,totalPharmasists,labScientists,totalLabScientists,patients,totalPatients, staffs,noOfStaffs} = adminUserManagement()


  useEffect(()=>{
      if(searchQuery){
          const filter =  staffs.filter((items)=> {
      ...
      {/*<div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            <Card className="p-4 flex items-center gap-3" onClick={()=>{setFilteredStaff(staffs)}}>
                <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center"><Users className="w-5 h-5 text-primary" /></div>
                <div><p className="text-xl font-bold">{noOfStaffs}</p><p className="text-xs text-muted-foreground capitalize">{'Total Staff'}s</p></div>
            ...
            <Card className="p-4 flex items-center gap-3" onClick={()=>{setFilteredStaff(pharmasists)}}>
                <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center"><Users className="w-5 h-5 text-primary" /></div>
                <div><p className="text-xl font-bold">{totalPharmasists}</p><p className="text-xs text-muted-foreground capitalize">{'pharmasist'}s</p></div>
            </Card>
            <Card className="p-4 flex items-center gap-3" onClick={()=>{setFilteredStaff(pharmasists)}}>
                <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center"><Users className="w-5 h-5 text-primary" /></div>
                <div><p className="text-xl font-bold">{totalDoctors}</p><p className="text-xs text-muted-foreground capitalize">{'Doctor'}s</p></div>
            </Card>
            <Card className="p-4 flex items-center gap-3" onClick={()=>{setFilteredStaff(nurses)}}>
                <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center"><Users className="w-5 h-5 text-primary" /></div>
                <div><p className="text-xl font-bold">{totalNurses}</p><p className="text-xs text-muted-foreground capitalize">{'Nurse'}s</p></div>
            </Card>
            <Card className="p-4 flex items-center gap-3" onClick={()=>{setFilteredStaff(labScientists)}}>
                <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center"><Users className="w-5 h-5 text-primary" /></div>
                <div><p className="text-xl font-bold">{totalLabScientists}</p><p className="text-xs text-muted-foreground capitalize">{'LabScientist'}s</p></div>
            </Card>
            <Card className="p-4 flex items-center gap-3" onClick={()=>{setFilteredStaff(clerks)}}>
                <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center"><Users className="w-5 h-5 text-primary" /></div>
                <div><p className="text-xl font-bold">{totalClerks}</p><p className="text-xs text-muted-foreground capitalize">{'Clerk'}s</p></div>
            </Card>
            <Card className="p-4 flex items-center gap-3" onClick={()=>{setFilteredStaff(accountants)}}>
                <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center"><Users className="w-5 h-5 text-primary" /></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredStaff?.map(s => {
         /* const allowances = s.allowances.reduce((a, all) => a + all.amount, 0);
          ...
          return (
            <Card key={s.id} className="p-5 hover:shadow-md transition-all" onClick={()=>{handleViewStaff(s)}}>
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{s.regID}</p>
                  <div className="flex gap-2 mt-2">
                    <Badge variant="outline" className="text-xs capitalize">{s.user_role}</Badge>
                    <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium border ${s.suspended === '0' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{s.suspended}</span>
                  </div>
                  <div className="mt-3 space-y-1 text-xs text-muted-foreground">
                    <p className="flex items-center gap-1"><Mail className="w-3 h-3" />{s.email}</p>
                    <p className="flex items-center gap-1"><Phone className="w-3 h-3" />{s.phone_no}</p>
                    <p>Dept: {s.user_role}</p>
