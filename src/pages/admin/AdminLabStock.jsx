...
import {adminUserManagement, drugStore, labStore} from '../../store/store.jsx';
import axiosClient from '../../service/axiosClient.js';

const AdminLabStock = () => {
    const navigate = useNavigate();
    const { drugs,pendingDrugs } = drugStore();
    const { labStock,labPendingStock } = labStore();
    const { pendingLabStock,approvedLabStock,pendingLabStockRequest,approvedLabStockRequest } = adminUserManagement();
    ...
    // Filter requests
    let displayedRequests = approvedLabStock
    if (filter === 'low') displayedRequests = approvedLabStock;
    else if (filter === 'outOfStock') displayedRequests = approvedLabStock;
    else if (filter === 'pending') displayedRequests = pendingLabStock;
    else if (filter === 'restockRequest') displayedRequests = approvedLabStockRequest;
    else if (filter === 'myRequest') displayedRequests = pendingLabStockRequest;
    else if (filter === 'pendingRequest') displayedRequests = pendingLabStockRequest;
...
*/    const filteredRequests = searchQuery
    ...
    const handleSubmitRequest = async () => {
        ...
        try {
            ...
            axiosClient.post('/addLabStockRequest', payload)
                .then(({data})=>{
    ...
    return (
        <div className="space-y-6">
            ...
            <PageHeader
                title="Lab Requests"
                subtitle="Manage and track Lab stock requests"
                breadcrumb={[
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                >
                    ...
                    <div className={'flex justify-center items-center flex-col'}>
                        <p className="text-2xl font-bold">{approvedLabStock.length}</p>
                        <p className="text-sm text-muted-foreground">Lab Stock</p>
                    </div>
                </Card>
                <Card
                    className="p-4 flex items-center gap-4 cursor-pointer hover:shadow-md transition-all"
                ...
                >
                    ...
                    <div className={'flex justify-center items-center flex-col'}>
                        <p className="text-2xl font-bold">{pendingLabStock.length}</p>
                        <p className="text-sm text-muted-foreground">Pending Lab Stock</p>
                    </div>
                </Card>
                <Card
                    className="p-4 flex items-center gap-4 cursor-pointer hover:shadow-md transition-all"
                    onClick={() => setFilter('pendingRequest')}
                >
                    <div className="w-12 h-12 rounded-lg bg-blue-100 flex items-center justify-center">
                        <Package className="w-6 h-6 text-blue-600" />
                    </div>
                    <div className={'flex justify-center items-center flex-col'}>
                        <p className="text-2xl font-bold">{pendingLabStockRequest.length}</p>
                        <p className="text-sm text-muted-foreground">Pending Restock Requests</p>
                    </div>
                </Card>
                <Card
                    className="p-4 flex items-center gap-4 cursor-pointer hover:shadow-md transition-all"
                    onClick={() => setFilter('restockRequest')}
                >
                    <div className="w-12 h-12 rounded-lg bg-blue-100 flex items-center justify-center">
                        <Package className="w-6 h-6 text-blue-600" />
                    </div>
                    <div className={'flex justify-center items-center flex-col'}>
                        <p className="text-2xl font-bold">{approvedLabStockRequest.length}</p>
                        <p className="text-sm text-muted-foreground">Approved Restock Requests</p>
                    </div>
                </Card>
                {/* <Card
                    className="p-4 flex items-center gap-4 cursor-pointer hover:shadow-md transition-all"
                ...
                <div className="flex gap-2 mb-4">
                    {['all', 'low', 'outOfStock','pending','restockRequest','pendingRequest'].map(f => (
                        <Button key={f} variant={filter === f ? 'default' : 'outline'} size="sm" onClick={() => setFilter(f)}>
                            {f === 'all' ? 'All' : f === 'low' ? 'Low Stock' : f === 'outOfStock' ? 'outOfStock' : f === 'restockRequest' ? 'restockRequest'  : f=== 'pendingRequest' ? 'pendingRequest'  : 'New Pending Item'}
                        </Button>
                    ))}
                </div>
                            ...
                            <table className="w-full text-sm">
                                ...
                                {(filter == 'restockRequest' || filter == 'myRequest' || filter == 'pendingRequest') ? (
                                         ...
                                         return (
                                            ...
                                            >
                                                ...
                                                <td className="p-3 font-medium">
                                                    {request.lab_stock.name || 'N/A'}
                                                </td>
                                                <td className="p-3">{request.quantity}</td>
                                                <td className="p-3">
                                                    <div className="flex items-center gap-2">
                                                        <User className="w-3 h-3 text-muted-foreground" />
                                                        {request.description || 'Unknown'}
                                                    </div>
                                                </td>
                                               {/* <td className="p-3">
                                    ...
                                    {filteredRequests?.map(request => {
                                        const statusConfig = getStatusConfig(request.status);
                                        const priorityConfig = getPriorityConfig(request.priority);                                        return (
                                            <tr
                                                key={request.id}
                                                className="border-b hover:bg-muted/50 transition-colors cursor-pointer"
                                                onClick={() => {
