...
const PharmacyPatientDetailsPage = () => {
    ...
    useEffect(()=>{
        setIsLoading(true)
        axiosClient.get(`/getPatientPrescriptionByRegID?regID=${searchParams.get('regID')}`)
            .then(({data})=>{
            ...
            <Card className="p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[500px] overflow-y-auto scrollbar-thin p-1">
                    {recentDrugSales?.length > 0 ? (
                        recentDrugSales.map((sale, index) => (
                            <div
                                key={sale.id}
                              ...
                              /*  onClick={() => navigate(`/doctor/diagnosisDetail/${sale.id}`)}*/
                            >
                            ...
                            <span className="text-xs font-semibold text-primary bg-primary/10 px-2.5 py-0.5 rounded-full">
                                #{index + 1}
                                    ...
                                    <StatusBadge status={sale.delievery_status || 'active'} size="sm" />
                                </div>
                                ...
                                <div className="mt-2 space-y-1 flex-1">
                                    ...
                                    <p className="text-xs text-muted-foreground flex items-center gap-1">
                                        <Stethoscope className="w-3 h-3" />
                                        <span> {sale.doctor?.user?.name || 'Unknown'}</span>
                                    </p>
                                    <p className="text-xs text-muted-foreground flex items-center gap-1">
                                ...
                                <div className="mt-3 pt-3 border-t flex items-center justify-between">
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span className="text-sm font-semibold">{sale.drug_stock?.length || 0}</span>
                                    ...
                                    <Button
                                        ...
                                        /*onClick={(e) => {
                                            e.stopPropagation();
                                            navigate(`/doctor/diagnosisDetail/${sale.id}`)
                                        }}*/
                                        onClick={() => {
...
export default PharmacyPatientDetailsPage;