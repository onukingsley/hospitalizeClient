...
const DiagnosisEdit = () => {
    ...
    useEffect(()=>{
        ...
        axiosClient.get(`/getDoctorsPatientDiagnosis?diagnosis_id=${searchParams.get('diagnosis_id')}`)
            .then(({data})=>{
                setSelectedDiagnosis(data.data)
                setSelectedVitals(JSON.parse(data.data.body_vitals))
    ...
    const handleGetDiagnosis = ()=>{
        setIsLoading(true)
        if (selectedVitals){
            setSelectedDiagnosis({...selectedDiagnosis, body_vitals: JSON.stringify(selectedVitals)})
        }
       ...
       /* if (!chiefComplaints || !provisionalDiagnosis) {
        ...
        const diagnosisData = {
            ...
            editPayload: {
                ...
                body_vitals: JSON.stringify( selectedVitals),
                ward_status : selectedDiagnosis?.ward_status,
                patients_complain: selectedDiagnosis?.patients_complain,
                initial_diagnosis : selectedDiagnosis?.initial_diagnosis,
                final_diagnosis : selectedDiagnosis?.final_diagnosis,
            }
        ...
        axiosClient.post('/updateDiagnosis', diagnosisData)
            .then(({data})=>{
                alert(data.message)
    ...
    return (
        <div className="space-y-6">
            ...
            <PageHeader
                title="New Diagnosis"
                subtitle="Create a comprehensive patient diagnosis"
                breadcrumb={[
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <Textarea
                                        ...
                                        value={selectedDiagnosis?.patients_complain}
                                        onChange={e => setSelectedDiagnosis({...selectedDiagnosis, patients_complain: e.target.value})}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <Input
                                            value={selectedDiagnosis?.initial_diagnosis}
                                            onChange={e => setSelectedDiagnosis({...selectedDiagnosis, initial_diagnosis: e.target.value})}
                                    ...
                                    <div>
                                        ...
                                        <Input
                                            value={selectedDiagnosis?.final_diagnosis}
                                            onChange={e => setSelectedDiagnosis({...selectedDiagnosis, final_diagnosis: e.target.value})}
                                    ...
                                    <div>
                                        ...
                                        <Select
                                            value={selectedDiagnosis?.ward_status}
                                            onValueChange={v => setSelectedDiagnosis({...selectedDiagnosis, ward_status: v})}
                                        >
                                ...
                                <div>
                                    ...
                                    <Textarea
                                        ...
                                        onChange={e => setSelectedDiagnosis({...selectedDiagnosis, description: e.target.value})}
                    ...
                    <TabsContent value="prescriptions">
                        <Card className="p-6">
                         ...
                         {/*   {selectedDiagnosis?.sales[0]?.drug_stock.length && (
                                <div className="space-y-2 mb-4 max-h-[300px] overflow-y-auto">
                                    {selectedDiagnosis?.sales[0]?.drug_stock.map((pres, i) => (
                                        <div key={i} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg hover:bg-muted transition-colors group">
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <Badge variant="outline" className="text-[10px]">{pres.pivot.dosage}</Badge>
                                                </div>
                                                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground mt-1">
                                                    <span>{pres.pivot.status}</span>
                                                    {pres.pivot.duration && <span>× {pres.pivot.duration} days</span>}
                                                    <span>• {pres.pivot.route}</span>
                                                    {pres.quantity > 0 && <span>• Qty: {pres.pivot.quantity}</span>}
                                                </div>
                                                {pres.pivot.instruction && (
                                                    <p className="text-xs text-muted-foreground mt-1">{pres.pivot.instruction}</p>
                                                )}
                                            </div>
                                            <Button
                                                type="button"
                         ...
                         {/*   {console.log(selectedDiagnosis?.sales.find((item)=>newPrescription?.sales_id == item.id))}*/}

                            {selectedDiagnosis?.sales?.length && (
                                <div className="space-y-2 mb-4 max-h-[300px] overflow-y-auto">
                                    {
                                     selectedDiagnosis?.sales.find((item)=>newPrescription?.sales_id == item.id)?.payment_status == 'unpaid' && selectedDiagnosis?.sales.find((item)=>newPrescription?.sales_id == item.id)?.doctor_id == doctor?.id  ?
                                         (
                                           <>
                                               <div className="flex items-center justify-between mb-4">
                                                   ...
                                                   <Badge variant="secondary" className="text-sm">
                                                       {selectedDiagnosis?.sales.find((item)=>newPrescription?.sales_id == item.id)?.drug_stock?.length} prescribed
                                                   </Badge>
                                               </div>
                                               {selectedDiagnosis?.sales.find((item)=>newPrescription?.sales_id == item.id)?.drug_stock?.map((pres,i)=>{
                                                   ...
                                                   return <>
                                                       ...
                                                       <div key={i}
                                                           ...
                                                           <div>
                                                               <div className="flex items-center gap-2">
                                                                   ...
                                                                   <Badge variant="outline"
                                                                          className="text-[10px]">{pres.pivot?.dosage}</Badge>
                                                               </div>
                                                               <div
                                                                   className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground mt-1">
                                                                   <span>{pres?.pivot?.status || 'active'}</span>
                                                                   {pres.pivot.duration &&
                                                                       <span>× {pres.pivot.duration} days</span>}
                                                                   <span>• {pres.pivot.route}</span>
                                                                   {pres.quantity > 0 && <span>• Qty: {pres.pivot.quantity}</span>}
                                                                   <span>• Sales ID: {newPrescription.sales_id}</span>
                                                               </div>
                                                               {pres.pivot.instruction && (
                                                                   <p className="text-xs text-muted-foreground mt-1">{pres.pivot.instruction}</p>
                                                               )}
                                                           ...
                                                           <Button
                                                               ...
                                                               size="sm"
                                                               //onClick={() => removePrescription(i)}
                                                               onClick={() => removePrescription(selectedDiagnosis?.sales.find((item)=>newPrescription?.sales_id == item.id), pres.id)}
                                                               className="opacity-0 group-hover:opacity-100 transition-opacity"
                                                           >
                                                               <Trash2 className="w-4 h-4 text-red-500"/>
                                                           </Button>
                                         ...
                                         (
                                             <>
                                                 <div className="flex items-center justify-between mb-4">
                                                     <Badge variant="secondary" className="text-sm">
                                                         {selectedDiagnosis?.sales.find((item)=>newPrescription?.sales_id == item.id)?.drug_stock?.length} Paid prescribed
                                                     </Badge>
                                                 </div>
                                                 {selectedDiagnosis?.sales.find((item)=>newPrescription?.sales_id == item.id)?.drug_stock?.map((pres,i)=>{
                                                     ...
                                                     return (
                                                         <div key={i}
                                                              className="flex items-center justify-between p-3 bg-muted/50 rounded-lg hover:bg-muted transition-colors group bg-red-50">
                                                             <div>
                                                                 <div className="flex items-center gap-2">
                                                                     <p className="font-medium text-sm">{pres.name}</p>
                                                                     <Badge variant="outline"
                                                             ...
                                                             <Button
                                                                 ...
                                                                 size="sm"
                                                                 //onClick={() => removePrescription(i)}
                                                                 onClick={() => alert('This prescription has already been paid for, hence cannot be edited')}
                                                                 className="opacity-0 group-hover:opacity-100 transition-opacity"
                                                             >
                                                                 <Trash2 className="w-4 h-4 text-red-500"/>
                                                             </Button>
...
{/*
                            {selectedDiagnosis?.sales?.length && (
                                ...
                                <div className="space-y-2 mb-4 max-h-[300px] overflow-y-auto">
                                    {
                                        selectedDiagnosis?.sales.map((sale, i) => {
                                      if (sale?.payment_status=='unpaid' && sale.doctor_id == doctor?.id){
                                            ...
                                            return <>
                                                <div className="flex items-center justify-between mb-4">
                                                    ...
                                                    <Badge variant="secondary" className="text-sm">
                                                        {totalprescribedlenght += sale?.drug_stock.length} prescribed
                                                    </Badge>
                                                </div>
                                            {
                                                sale.drug_stock.map((pres,i)=>{
                                                    return (
                                                        <div key={i}
                                                             className="flex items-center justify-between p-3 bg-muted/50 rounded-lg hover:bg-muted transition-colors group">
                                                            <div>
                                                            ...
                                                            <Button
                                                                ...
                                                                size="sm"
                                                                //onClick={() => removePrescription(i)}
                                                                onClick={() => removePrescription(sale, pres.id)}
                                                                className="opacity-0 group-hover:opacity-100 transition-opacity"
                                                            >
                                                                <Trash2 className="w-4 h-4 text-red-500"/>
                                                            </Button>
                                      ...
                                      else if(sale?.payment_status=='paid' && sale.doctor_id == doctor?.id){
                                          return <>
                                              <div className="flex items-center justify-between mb-4">
                                                  <Badge variant="secondary" className="text-sm">
                                                      {totalprescribedlenght += sale?.drug_stock.length} Paid prescribed
                                                  </Badge>
                                              </div>
                                              {
                                                  sale.drug_stock.map((pres,i)=>{
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                                ...
                                <div>
                                    <Label className="text-xs">Sales ID</Label>
                                    <Select
                                ...
                                <div>
                                    <Label className="text-xs">Sales ID</Label>
                                    <Select
                                        value={newPrescription.sales_id}
                                        onValueChange={v => setNewPrescription(prev => ({ ...prev, sales_id: v }))}
                                    >
                                        <SelectTrigger className="h-9">
                                        ...
                                        <SelectContent>
                                            {selectedDiagnosis?.sales?.length > 0 ? (
                                                selectedDiagnosis.sales.map(f => (
                                                    <SelectItem key={f.id} value={String(f.id)}>
                                                        <div className="flex flex-col">
                                                            <span className="font-medium">Sale #{f.id}</span>
                                                            <span className="text-xs text-muted-foreground">
                            Doctor: {f.doctor?.user?.name || 'N/A'} •
                            Status: {f.payment_status || 'pending'} •
                            Items: {f.drug_stock?.length || 0}
                        </span>
                                                        </div>
                                                    </SelectItem>
                                                ))
                                ...
                                <div className="col-span-4 flex justify-end">
                                    <Button
                                        ...
                                        onClick={() => {
                                            if (!newPrescription?.sales_id) {
                                                alert('Please select the Prescription in other to add new Prescription')
                                            }else {
                                                addPrescription()
                                            }
                                        }}
                    ...
                    <TabsContent value="lab">
                        <Card className="p-6">
                            ...
                            {selectedDiagnosis?.lab_test.length>0 && (

                                <div className="flex flex-wrap gap-2 mb-4">

                                    {selectedDiagnosis?.lab_test.find(item=>item.id == newPrescription?.labTest_id)?.lab_test_payment_status === 'unpaid' ? (

                                            <>
                                                {selectedDiagnosis?.lab_test.find(item=>item.id == newPrescription?.labTest_id)?.rates.map((lab)=>{
                                                   /* console.log(selectedDiagnosis?.lab_test.find(item=>item.id == newPrescription?.labTest_id))*/
                                                    return (
                                                        <Badge
                                                            key={lab.id}
                                                            variant="secondary"
                                                        ...
                                                        >
                                                            ...
                                                            <button
                                                                type="button"
                                                                onClick={() => removeLabOrder(selectedDiagnosis?.lab_test.find(item=>item.id == newPrescription?.labTest_id),lab.id)}
                            ...
                            <div className='flex w-full space-x-10'>
                                ...
                                <div>
                                    <Label className="text-xs">Lab test ID</Label>
                                    <Select
                                        value={newPrescription.labTest_id}
                                        onValueChange={v => setNewPrescription(prev => ({ ...prev, labTest_id: v }))}
                                    >
                                        <SelectTrigger className="h-9">
                                            <SelectValue placeholder="Select a sale" />
                                        </SelectTrigger>
                                        <SelectContent>
                                            {selectedDiagnosis?.lab_test?.length > 0 ? (
                                                selectedDiagnosis.lab_test.map(f => (
                                                    <SelectItem key={f.id} value={String(f.id)}>
                                                        <div className="flex flex-col">
                                                            <span className="font-medium">LabTest #{f.id}</span>
                                                            <span className="text-xs text-muted-foreground">
                            Doctor: {f.doctor?.user?.name || 'N/A'} •
                            labtest ID: {f.id || 'pending'}
                        </span>
                                                        </div>
                                                    </SelectItem>
                                                ))
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-[300px] overflow-y-auto">
                                {rates.map(test => {
                                    /*console.log(test,'this is the tesst')*/
                                    ...
                                    return (
                                        <button
                                            ...
                                            type="button"
                                            onClick={() => {
                                                if (!newPrescription?.labTest_id) {
                                                   alert('Please select the LabTestID in other to add new Test')
                                                }else {
                                                    addLabOrder(test.id, test.title, test)
                                                }
                                            }}
