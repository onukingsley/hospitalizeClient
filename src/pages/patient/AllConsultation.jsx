...
import { Search, Stethoscope, FileText, Activity, Thermometer, Heart, Gauge, Check } from 'lucide-react';
import {
    Dialog,
    DialogContent,
    DialogHeader,
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                        ...
                                        <div className="flex items-center gap-2 flex-shrink-0">
                                            <Button
                                                ...
                                                onClick={() => navigate(`/doctor/patientDetail/${dx?.patient?.user?.regID}`)}
                                            >
                                                View Patient
                                            </Button>
    ...
    const PendingConsultation = ({
        ...
        const handleStatusUpdate = (doctorId, consultation_id, consultation, regID) => {
            ...
            if (consultation.payment_status == 'unpaid'){
                alert('Patient is yet to make Payment for this consultation')
            ...
            axiosClient.post('/updateDoctorConsultation',payload)
                .then((data)=>{
                    showToast('Consultation Update',data.message)
                    removeAwaitingConsultation(consultation)
                    addConsultation(consultation)

                    navigate(`/doctor/patientDetail/${regID}?consultation_id=${consultation_id}`);
        ...
        const handleViewPatient = (regID) => {
            if (onViewPatient) {
                ...
                navigate(`/doctor/patientDetail/${regID}`);
            }
        };
            ...
            <div className="space-y-3">
                {isLoading ? (
                    <div className="flex items-center justify-center py-12">
                        <div className="flex flex-col items-center gap-3">
                            <div className="w-8 h-8 border-4 border-primary/30 border-t-primary rounded-full animate-spin" />
                            <p className="text-sm text-muted-foreground">Searching for diagnoses. Please wait...</p>
                        </div>
                    </div>
                ) : (
                    <>
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                                            <div className="flex items-start gap-3 flex-1 min-w-0">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <p className="text-xs text-muted-foreground">
                                                        Diagnosis #{dx.id} • {formatDate(dx.created_at)}
                                                    ...
                                                    <div className="flex flex-wrap gap-2 mt-2">
                                                        ...
                                                        <Badge variant={isOwnConsultation ? 'default' : 'outline'} className="text-xs">
                                                            {isOwnConsultation ? '✓ My Patient' : 'Pending'}
                                                        </Badge>
                                                    </div>
                                                </div>
                                            </div>
                                                    ...
                                                    <AlertDialog>
                                                        ...
                                                        <AlertDialogContent className="p-2 bg-card text-card-foreground flex flex-col gap-6 rounded-xl border py-6 shadow-sm">
                                                            <AlertDialogHeader>
                                                                ...
                                                                <AlertDialogDescription>
                                                                     Accept this consultation for
                                                                    <span className="font-semibold"> {dx?.patient?.user?.name}</span>?
                                                                    <br />
                                                                    <span className="text-xs text-muted-foreground mt-1 block">
                                                                    This will assign you as the attending doctor for this patient.
                                                                </span>
                                                                </AlertDialogDescription>
                                                            </AlertDialogHeader>
                                                            <AlertDialogFooter>
                                                                ...
                                                                <AlertDialogAction
                                                                    ...
                                                                    className="bg-green-600 p-2 rounded rounded-lg text-white hover:bg-green-700"
                                                                >
                                                                    Confirm Acceptance
                                                                </AlertDialogAction>
                                                            </AlertDialogFooter>
        ...
        <div className="space-y-6">
          ...
          {/*  <Card className="p-4">
            ...
            <Tabs defaultValue="pending consultation">
                <TabsList>
                    <TabsTrigger value="My consultation">My consultation ({consultations?.length})</TabsTrigger>
                ...
                <TabsContent value="My consultation"><DiagnosisList items={ consultations } /></TabsContent>
