...
const AllDiagnosis  = () => {
    ...
    useEffect(()=>{
        ...
        if(searchParams.get('patient_id')){
            /* setSearchQuery(searchParams.get('patient_id'))*/
            setUrlParams(searchParams.get('patient_id'))
            setIsLoading(true)
            axiosClient.get(`/getDoctorsPatientDiagnosis?details=${searchParams.get('patient_id')}`)
                .then(({data})=>{
                    setIsLoading(false)
   ...
   // const myDiagnoses = diagnoses.filter(d => d.doctorId === 'DOC001');
            ...
            : (
                   <>
                       {items?.map(dx => {
                           ...
                           return (
                               <Card key={dx?.id} className={`p-4 hover:shadow-md transition-all cursor-pointer ${dx?.doctor_id === doctor.id ? 'bg-gray-100':'bg-white' }`} onClick={() =>  navigate(`/doctor/diagnosisDetail/${dx?.id}`)}>
    ...
    return (
        <div className="space-y-6">
            <PageHeader
                title={`Patient Diagnosis  ${urlParams? urlParams:''}`}
                ...
                <div className="relative">
                   {/* <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />*/}
                    <div className='flex gap-x-2' >
                        <Input placeholder="Search by patient ID, diagnosis ID, or patient name..." value={searchQuery} onChange={e => {
            ...
            <Tabs defaultValue="My Diagnosis">
                <TabsList>
                    <TabsTrigger value="My Diagnosis">My Diagnosis ({doctorsDiagnosis?.length})</TabsTrigger>
                    <TabsTrigger value="outPatients">outPatients ({Object.values(outPatientDiagnosis)?.length})</TabsTrigger>
                    <TabsTrigger value="inPatients">inPatients ({Object.values( inwardDiagnosis)?.length})</TabsTrigger>
                </TabsList>
                <TabsContent value="My Diagnosis"><DiagnosisList items={searchDiagnosis?.length>0 ? searchDiagnosis : doctorsDiagnosis } /></TabsContent>
                <TabsContent value="outPatients"><DiagnosisList items={Object.values(outPatientDiagnosis) || []} /></TabsContent>
                <TabsContent value="inPatients"><DiagnosisList items={Object.values(inwardDiagnosis) || []} /></TabsContent>
            </Tabs>
