                ...
                : (
                    <>
                        {items?.map(dx => {
                            ...
                            return (
                                <Card key={dx.id} className={`p-4 hover:shadow-md transition-all cursor-pointer ${dx.doctor_id === doctor.id ? 'bg-gray-100':'bg-white' }`} onClick={() =>  navigate(`/doctor/patientDetail/${dx.regID}`)}>
    ...
    return (
        <div className="space-y-6">
            <PageHeader
                ...
                actions={<Button onClick={() => navigate('/doctor/diagnosis/new?patient_regID')}><Stethoscope className="w-4 h-4 mr-2" />New Diagnosis</Button>}
            />
