...
const Pnl = () => {
    ...
    const {totalRevenue,totalConsultation,deptChart,totalDrugSale,totalExpenses,totalLabTest,totalSalary,pnlChart, payments,
        setPayment,setCreditPayment,setDebitPayment,setRate,setTotalRevenue,setTotalExpenses,setTotalSalary,setTotalConsultation,setTotalDrugSale,setTotalLabTest,setDeptChart,setPnlChart,drugPnlChart,labPnlChart,setTotalDrugExpenses,setLabPnlChart,setDrugPnlChart,setTotalLabExpenses,setGrossMargin,setNetProfit,totalDrugExpenses,totalLabExpenses,grossMargin,netProfit,expensesChart,setExpensesChart,setTotalEnrollment,totalEnrollment
    } = paymentStore()

    const {consultations} = diagnosisStore()
    ...
    const expenseBreakdown = [
        { name: 'Lab Expenses', value: totalLabExpenses },
        { name: 'Payroll', value: totalSalary },
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <StatCard title="Lab Expenses" value={parseInt(totalLabExpenses).toLocaleString()} icon={<AlertCircle className="w-5 h-5" />} color="bg-yellow-500" />
                <StatCard title="Lab Revenue" value={parseInt(totalLabTest).toLocaleString()} icon={<AlertCircle className="w-5 h-5" />} color="bg-yellow-500" />
                <StatCard title="Drug Sales Revenue" value={parseInt(totalDrugSale).toLocaleString()} change={8.2} changeLabel="vs last month" icon={<TrendingUp className="w-5 h-5" />} color="bg-blue-500" />
                <StatCard title="Drug Expenses" value={parseInt(totalDrugExpenses).toLocaleString()} change={8.2} changeLabel="vs last month" icon={<TrendingUp className="w-5 h-5" />} color="bg-blue-500" />
                ...
                <StatCard title="Enrollment" value={parseInt(totalEnrollment).toLocaleString()} change={8.2} changeLabel="vs last month" icon={<TrendingUp className="w-5 h-5" />} color="bg-blue-500" />
                <StatCard title="Monthly Payroll" value={parseInt(totalSalary).toLocaleString()} icon={<Wallet className="w-5 h-5" />} color="bg-purple-500" />
                <StatCard title="Payment Count" value={parseInt(payments.length)} icon={<Wallet className="w-5 h-5" />} color="bg-purple-500" />
                <StatCard title="Gross Margin" value={parseInt(grossMargin?.toFixed(2)) + '%'}  icon={<Wallet className="w-5 h-5" />} color="bg-purple-500" />
                <StatCard title="Net Profit" value={parseInt(netProfit).toLocaleString()} icon={<Wallet className="w-5 h-5" />} color="bg-purple-500" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card className="p-6">
                    <h3 className="text-lg font-semibold mb-4">Lab Revenue vs Expenses Trend</h3>
                    <ResponsiveContainer width="100%" height={250}>
                        <AreaChart data={labPnlChart }>
                            <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                            <XAxis dataKey="date" stroke="#94a3b8" fontSize={12} />
                            <YAxis stroke="#94a3b8" fontSize={12} tickFormatter={(v) => `₦${(v/1000000).toFixed(1)}M`} />
                            <Tooltip formatter={(value) => value} />
                ...
                <Card className="p-6">
                    <h3 className="text-lg font-semibold mb-4">Drug Revenue vs Expenses Trend</h3>
                    <ResponsiveContainer width="100%" height={250}>
                        <AreaChart data={drugPnlChart }>
                            <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                            <XAxis dataKey="date" stroke="#94a3b8" fontSize={12} />
                            <YAxis stroke="#94a3b8" fontSize={12} tickFormatter={(v) => `₦${(v/1000000).toFixed(1)}M`} />
                            <Tooltip formatter={(value) => value} />
