...
const PatientEnrollment = () => {
    ...
    const handleSearchPatient = async () => {
        ...
        setIsLoading(true);
        try {
            const response = await axiosClient.get(`/getPatientEnrollment?details=${searchRegID}`);
            ...
            if (response.data.data) {
                ...
                setPaymentAmount(response.data.data.rate)
    ...
    const handlePayment = async () => {
        ...
        const payload  = {
            ...
            rates_id : paymentAmount.id,
            amount : paymentAmount.amount
        }

        axiosClient.post('/enrollmentPayment', payload)
            .then(({data})=>{
            ...
            }).catch(e=> {setMessage(e.message); setShowMessage(true)})
    ...
    const renderPaymentItem = (item, type, icon, label,rate) => {
        ...
        return (
            ...
            >
                <div className="flex items-start gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <StatusBadge status={item?.patient.enrollment_status} size="sm" />

                        </div>
                        <p className="text-sm text-muted-foreground">Patient ID: {item.regID}</p>
                ...
                <div className="text-right">
                    <p className={`text-lg font-bold  text-primary `}>{formatCurrency(rate)}</p>
                    <Button
                        size="sm"
                        variant="outline"
                        className="mt-1"
    ...
    return (
        <div className="space-y-6">
            ...
            <PageHeader
                ...
                subtitle="Payment to complete Patient Registration"
                actions={
                    <div className="flex items-center gap-2">
            ...
            <MessageModal
                message={message} setMessage={setMessage} setShowMessage={setShowMessage} showMessage={showMessage}
            />
                                    ...
                                    <div className="p-4 bg-muted/30 rounded-lg space-y-2">
                                        ...
                                        <div className="flex justify-between">
                                            ...
                                            <span className={`text-xl font-bold  ${paymentAmount.toString().charAt(0) == '-' ? 'text-red-500' : 'text-primary'} `}>
                                                {formatCurrency(paymentAmount.amount)}
